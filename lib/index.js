...
const byejack = async ({ package, warningThresholdInDays }) => {
    ...
    const actual = await arb.loadActual()
    const ideal = await arb.buildIdealTree({ add: [package], saveType: 'dev'})
    const diffResult = Diff.calculate({ actual, ideal })
    const added = diffResult.children.filter(isAdded).map(child => getNameAndVersionFromTarUrl(child.resolved))
    console.log(`${added.length} packages would be added by installing "${package}"` )
    const allAuditResults = await Promise.all(added.map(async package => {
        let published = null
        try {
